...
Corpus federation
([ADR-089](https://github.com/itsthelore/rac-core/blob/main/rac/decisions/adr-089-corpus-federation.md))
...
```
                       org-standards repo (main)
                                 │
                     shared Lore server (shared-server.md:
...
`rac quickstart`, artifacts under `rac/`, validated and gated like any other.
...
- **Curate the live constraints, not the archive.** A few hundred binding,
  ...
  Seed it with [`rac ingest`](cli.md#ingest) from what already exists, and
  promote only what an owner ratifies as live.
- **Govern it like code.** Changes land by pull request behind required
  review; use `CODEOWNERS` on `rac/decisions/` so every standard has an
  accountable owner.
...
Operationally the org endpoint **is** a shared Lore server — follow
[Operating a Shared Lore Server](shared-server.md) as written: the container,
...
- **Managed rules blocks for non-MCP clients.** Generate agent-rules blocks
  ...
  ```bash
  rac export /path/to/org-corpus --agent-rules
  ```
...
- **Not federation.** There is no cross-corpus resolution, validation,
  ...
  [federation programme](https://github.com/itsthelore/rac-core/blob/main/rac/roadmaps/corpus-federation.md),
...
([ADR-088](https://github.com/itsthelore/rac-core/blob/main/rac/decisions/adr-088-enterprise-profile-scaffold.md)).
