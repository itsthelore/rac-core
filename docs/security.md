...
- **No network access.** `rac` reads and writes the local filesystem only. None
  ...
  invariant ([ADR-002](https://github.com/itsthelore/rac-core/blob/main/decisions/decisions/adr-002-ai-optional.md)):
...
- **No telemetry by default; provably off for regulated installs.** RAC collects
  ...
  content ([ADR-041](https://github.com/itsthelore/rac-core/blob/main/decisions/decisions/adr-041-anonymous-usage-ping.md)) —
  which stays off unless you opt in. A regulated install can prove it stays off at
  runtime with `decided telemetry off --enterprise`: a tamper-evident hard-lock that
  forces the ping off and refuses re-enabling until explicitly unlocked
  ([ADR-086](https://github.com/itsthelore/rac-core/blob/main/decisions/decisions/adr-086-air-gap-and-enterprise-telemetry-lock.md)).
  AI-assisted authoring is opt-in and bring-your-own-credentials
  ([ADR-035](https://github.com/itsthelore/rac-core/blob/main/decisions/decisions/adr-035-byo-ai-credentials.md));
...
- **Optional read-access audit, local-only.** Regulated installs can record who
  ...
  never bodies — by enabling an `audit:` stanza in `.decided/config.yaml`
...
```
local Markdown (decisions/)  ->  rac (pure, in-process analysis)  ->  local output
...
| `pyyaml` | Frontmatter and `.decided/config.yaml` parsing. |
...
- [ADR-002](https://github.com/itsthelore/rac-core/blob/main/decisions/decisions/adr-002-ai-optional.md) — deterministic, AI-optional core.
- [ADR-035](https://github.com/itsthelore/rac-core/blob/main/decisions/decisions/adr-035-byo-ai-credentials.md) — bring-your-own AI credentials.
- [ADR-041](https://github.com/itsthelore/rac-core/blob/main/decisions/decisions/adr-041-anonymous-usage-ping.md) — the consent-gated, content-free anonymous ping.
- [ADR-084](https://github.com/itsthelore/rac-core/blob/main/decisions/decisions/adr-084-read-access-audit-recorder.md) — the local-only, default-off read-access audit log.
- [ADR-086](https://github.com/itsthelore/rac-core/blob/main/decisions/decisions/adr-086-air-gap-and-enterprise-telemetry-lock.md) — air-gap posture and the enterprise telemetry hard-lock.
