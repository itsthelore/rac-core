...
> Update (`rac-editors-buildout`): the extension now lives in `rac-editors`
> (ADR-092), so the `VSCE_PAT` / `OVSX_PAT` secrets land there, not in the
> archived `lore-vscode`. See `decisions/roadmaps/rac-editors-buildout.md`.
...
4. In `lore-vscode`, repoint `package.json` from `"@decisions/sdk": "file:../rac-sdk"`
   to the published `"@itsthelore/asdecided-sdk": "^<version>"`; move
...
2. **Seed with history:**
   ...
   # lore-gatekeeper (the pr-gate action = decided gate)
