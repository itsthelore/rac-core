...
| **`VSCE_PAT`** | Azure DevOps PAT, scope *Marketplace → Manage*; add as a secret in `lore-extensions`. | v0.22.5 |
| **OpenVSX namespace** | Create the namespace = publisher on open-vsx.org (Eclipse account), sign the publisher agreement; generate a token → **`OVSX_PAT`** secret in `lore-extensions`. | v0.22.5 |
...
2. Create `itsthelore/lore-extensions` and `itsthelore/rac-sdk-ts`.
3. **Seed each with history:**
   ...
   git remote add origin <lore-extensions-url> && git push -u origin main
   ```
4. In `lore-extensions`, repoint `package.json` from `"@rac/sdk": "file:../rac-sdk"`
...
6. **Publish the extension:** push tag `vscode-v0.1.0` in `lore-extensions` →
