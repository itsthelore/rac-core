"""Agent-rules projection — ``rac export --agent-rules [--check]`` (ADR-067).

Distils the *live* corpus into a small, stable "managed block" that per-client
agent-context files (``CLAUDE.md``, ``AGENTS.md``, ``.cursor/rules``,
``.github/copilot-instructions.md``) carry, so an AI coding agent sees the
decisions the team has already settled and stops re-litigating them.

ADR-067 fixes the boundary this service honours:

- The projection is *distilled*, not the whole corpus: one pointer line per live
  decision (canonical identifier + title, optionally its category), never the
  decision bodies. The agent reaches the ``lore`` MCP tools for ground truth;
  inlining the corpus is an explicit non-goal (attention budget).
- "Live" is deterministic and structural (ADR-002): a decision is live when the
  first non-empty line of its ``## Status`` is *Accepted*. No semantic verdict
  enters the engine.
- The block is drift-guarded: it embeds a sha256 provenance digest over the
  ...
  the same corpus are byte-identical.

Generation only ever replaces the managed block; prose a user keeps outside the
markers is preserved verbatim. The JSON shapes here are a stable contract
(ADR-007).
"""
...
# The block distils decisions only. ADR-067 scopes it to bans + closed-decision
# pointers; roadmap scope-fences are a deliberate future non-goal here.
_DECISION_TYPE = "decision"

# The live status, matched case-insensitively against the first non-empty line
# of ``## Status`` — a string match, not a semantic judgement.
_LIVE_STATUS = "accepted"

# Stable managed-block markers. HTML comments survive Markdown rendering, so the
# block stays invisible in a rendered file while remaining machine-locatable.
# The BEGIN marker carries the digest so ``--check`` compares without re-hashing.
_BEGIN_PREFIX = "<!-- BEGIN RAC MANAGED BLOCK (digest: "
_BEGIN_SUFFIX = ") -->"
_END_MARKER = "<!-- END RAC MANAGED BLOCK -->"

# In-block header, so a human opening the file knows what owns it. No timestamp,
# so regenerating an unchanged corpus is byte-identical.
_GENERATED_HEADER = (
...
class AgentRulesTarget:
    ...
    ``client`` is the stable ``--client`` selector; ``path`` is the file's
...
# The four targets in deterministic (selector) order. Cursor uses a single
# plain ``.cursor/rules`` file carrying the block; the others are Markdown.
...
def targets_for(clients: list[str] | None) -> list[AgentRulesTarget]:
    ...
    Always in ``TARGETS`` order regardless of the order selectors were given, so
    output never depends on argument order.
    """
    if not clients:
        return list(TARGETS)
    wanted = set(clients)
...
def unknown_clients(clients: list[str] | None) -> list[str]:
    """Selectors with no matching target (the caller turns these into a usage error)."""
    if not clients:
        return []
...
class AgentRulesEntry:
    ...
    A pointer, never a body: the "already decided / ruled out" digest the agent
    reads, with the decision itself reachable through the ``lore`` tools.
    """
...
class AgentRulesProjection:
    """The deterministic projection of the live corpus (ordered entries + digest).

    ``digest`` is a sha256 over the canonical serialization of the entries, so
    the same corpus state yields the same digest regardless of how the block is
    rendered for a given client.
    """
...
def _first_line(product: Product, section: str) -> str | None:
    """First non-empty, stripped line of ``## {section}``, or ``None`` when absent.

    The one place the "first meaningful line of a section" idiom lives; the
    status and category readers below build on it.
    """
...
def artifact_status(product: Product) -> str:
    """First non-empty line of ``## Status`` **as authored** (empty when absent).

    Case is preserved deliberately: consumers (``recency``, the MCP provenance
    surface) show the status verbatim, so ``Accepted`` must not be folded to
    ``accepted`` here. The casefolded liveness check below wraps this reader,
    keeping a single source for the status line while the two callers differ
    only in casing.
    """
    return _first_line(product, "status") or ""


def _category(product: Product) -> str | None:
    """First non-empty line of ``## Category`` (the decision's classification)."""
    return _first_line(product, "category")


def is_live_decision(product: Product) -> bool:
    """Public liveness predicate — the decision's status is *Accepted* (ADR-067).
    ...
    the two surfaces can never disagree. Structural only: a case-insensitive
    match of the first ``## Status`` line against *Accepted*. A retired status
    (Superseded/Deprecated) simply is not *Accepted*, so the same compare
    excludes it — no separate retired-set check is needed.
    """
    return artifact_status(product).casefold() == _LIVE_STATUS


def _canonical_payload(entries: list[AgentRulesEntry]) -> str:
    """Canonical, formatting-independent serialization the digest hashes.

    ``sort_keys`` + compact ``separators`` pin byte-for-byte stability; the
    entries are already ordered, so the digest depends only on corpus content.
    """
    return json.dumps(
...
def build_agent_rules_block(directory: str, recursive: bool = True) -> AgentRulesProjection:
    ...
    Walks the corpus once, keeps every live (Accepted) decision as a one-line
    pointer, orders deterministically by canonical identifier, and computes the
    provenance digest. An empty corpus yields an empty projection whose digest is
    that of the empty entry list — a valid, stable state.
    """
    ...
    for entry in walk_corpus(directory, recursive=recursive):
        if entry.artifact_type != _DECISION_TYPE:
            continue
        if not is_live_decision(entry.product):
            continue
    ...
    # Deterministic order: identifier case-insensitive, then exact for ties.
...
def render_managed_block(projection: AgentRulesProjection) -> str:
    ...
    Identical for every target: plain Markdown that lives equally well in a
    ``.md`` file or a Cursor rules file. The digest rides in the BEGIN marker so
    ``--check`` reads it without re-hashing the file.
    """
    lines = [
...
def embedded_digest(file_text: str) -> str | None:
    ...
    Reads only the marker, never re-hashing the file — the cheap comparison the
...
def merge_managed_block(existing: str | None, block: str) -> str:
    ...
    user's prose before and after is kept byte-for-byte. When there is no block,
    the new one is appended after the existing content (or stands alone in a
    fresh file). The result always ends with exactly one trailing newline.
    """
...
def generate_agent_rules(
    ...
    created, updated, or already in sync. A write is skipped when the file's
