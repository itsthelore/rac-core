...
`rac new`, `rac validate`) to one by composing the existing identity and creation
capabilities: it establishes the repository key (reusing ``init_repository``) and
scaffolds a single starter artifact (reusing ``create_artifact``) under the
conventional ``rac/<family>/`` path. The CLI stays a thin adapter, and any future
consumer (Explorer, an IDE action) shares one onboarding path.

The scaffold is a deliberately narrow exception to "RAC is not a content store"
(ADR-024), recorded in ADR-044: it writes exactly one artifact, only when the
corpus is empty, as the unmodified canonical template body plus a system-assigned
opaque id. A corpus that already holds any recognised artifact is refused before
anything is written.
...
class CorpusNotEmpty(RACError):
    ...
    Onboarding writes a starter artifact only into an empty corpus (ADR-044); an
    existing corpus is never scaffolded over.
    """
...
def _family_dir(artifact_type: str) -> str:
...
def quickstart(
    ...
    *,
    ...
    Validates the artifact type first (cheap), refuses a non-empty corpus before
    writing anything, then establishes the key and creates the starter artifact
    at ``<directory>/rac/<family>/first-<type>.md``. ``id_generator`` is
    injectable for deterministic tests.

    Raises :class:`~rac.core.templates.TemplateNotFound` for an unknown type,
    :class:`CorpusNotEmpty` when the corpus already holds a recognised artifact,
    and the :mod:`~rac.services.init` errors for key problems — in every refusal
    case nothing is written.
    """
    load_template(artifact_type)  # validate the type before any side effect

    # Walk the corpus once: the same scan that enforces the empty-corpus rule also
    # supplies the existing-ID set the starter artifact's identity check needs, so
    # cold start does not re-walk the tree inside create_artifact (ADR-044).
    ...
    existing_ids = frozenset(entry.id.upper() for entry in index.artifacts)

    init_result = init_repository(directory, key=key)

    art_dir = Path(directory) / "rac" / _family_dir(artifact_type)
    art_dir.mkdir(parents=True, exist_ok=True)
    out_path = art_dir / f"first-{artifact_type}.md"
    created_artifact = create_artifact(
        artifact_type, str(out_path), id_generator=id_generator, _existing_ids=existing_ids
    )
