...
Stands in for the stock Footer (DESIGN-visual-system): compact inverse-video
key chips on the left, context-sensitive to the focused panel (LazyGit UX),
with the health chip and link count right-aligned. Every chip carries a text
label, so nothing depends on the chip styling alone (ADR-028).
"""
...
# Focused region → the key hints that matter there. The screen names the region
# as focus moves; an unknown region falls back to "home".
...
def key_chips(pairs: tuple[tuple[str, str], ...]) -> Text:
    ...
    Each chip is a space-padded key in reverse video, its label trailing after
    a space, chips separated by two spaces. Shared with the confirm-write modal
    so every surface speaks the same hint language (DESIGN-visual-system).
    """
    text = Text()
...
class StatusLine(Horizontal):
    """Key chips on the left, the health chip and link count on the right."""
