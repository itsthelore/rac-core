...
the Product AST (see :mod:`asdecided.core.models`) is the internal model that
...
:data:`__all__` is importable directly from ``rac`` —
    ...
    from asdecided import validate_directory, collect_stats, RACError

    result = validate_directory("rac/")
    if not result.ok:
        ...

Every error a public function raises derives from :class:`asdecided.errors.RACError`,
...
:data:`__all__` (modules under :mod:`asdecided.core`, ``asdecided.cli``, output renderers)
...
from asdecided.services import (
    CreatedArtifact,
    artifact_recency,
    build_corpus_export,
    build_inspection,
...
try:
    ...
    # baked into the installed distribution. Keeps `decided --version` in sync.
