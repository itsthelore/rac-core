"""Bundled git hook registry — `decided hook` (v0.13.4).

The bundled hook set is a static registry of named git hooks with one-line
descriptions, surfaced by `decided hook list` and installable by style. Hook
content ships as package resources under :mod:`asdecided.hooks` and is loaded with
``importlib.resources``, mirroring how skills ship under :mod:`asdecided.skills`
(ADR-021), so installation works from an installed wheel without the
repository, network, or AI.

Two failure modes are deliberately distinct, mirroring :mod:`asdecided.core.skills`:
...
# Bundled hooks, in registry order. `decided hook install` defaults to the first
# (post-commit advisory); `decided hook list` enumerates them.
BUNDLED_HOOKS = (
    HookSpec(
        style="post-commit",
...
def load_hook(style: str) -> bytes:
    ...
    resource = resources.files("asdecided.hooks").joinpath(f"{style}.sh")
    try:
