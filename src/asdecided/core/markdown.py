"""Turn a Markdown requirement file into a :class:`~asdecided.core.models.Product` AST.
...
not enforce any rules. All rule-checking lives in :mod:`asdecided.core.validation`, so that
        ...
        return _degraded_product(
            ...
            [
                Issue(
                    ...
                    "(set DECIDED_MAX_FILE_BYTES to raise it)",
                    1,
                )
            ],
        )
...
def _oversize_issue(cap: int) -> Issue:
    return Issue(
        "error",
        "artifact-oversize",
        f"artifact exceeds the {cap}-byte file cap (set DECIDED_MAX_FILE_BYTES to raise it)",
        1,
    )
