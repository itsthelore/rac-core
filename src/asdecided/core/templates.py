"""Canonical artifact template registry — `decided new` / `decided templates` (v0.7.10).

The supported template set is derived from :data:`asdecided.core.artifacts.ARTIFACT_SPECS`
...
package resources under :mod:`asdecided.templates` and is loaded with
...
def load_template(artifact_type: str) -> str:
    ...
    resource = resources.files("asdecided.templates").joinpath(f"{artifact_type}.md")
    try:
