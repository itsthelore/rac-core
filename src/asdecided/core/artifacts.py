...
nothing else: `decided inspect` (v0.4) *consumes* them, and future capabilities
...
class ArtifactSpec:
    ...
    # Short authoring hints per normalized section name, surfaced by `decided improve
    ...
    # ``asdecided.core.identity.artifact_identifier`` before falling back to the filename
    ...
    # ``decided schema --template`` / ``decided improve --template``. Spec-driven so
...
def _registry() -> tuple[tuple[ArtifactSpec, ...], dict[str, str]]:
    ...
    raw = resources.files("asdecided.spec").joinpath("artifact-specs.json").read_text(encoding="utf-8")
...
# One-line descriptions for every relationship section, surfaced by `decided schema`.
# Relationship sections deliberately carry no ``guidance`` — guidance gates
# `decided improve`, and relationships must stay out of improve and templates. The
# relationship-section vocabulary and its canonical ordering live in
# :mod:`asdecided.services.relationships`; this map holds only the human-facing
