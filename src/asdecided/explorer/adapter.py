...
Explorer touches ``asdecided.services`` / ``asdecided.core``; widgets depend on
:mod:`asdecided.explorer.state` only. This module never imports Textual, so it is
...
from asdecided.core.frontmatter import split_frontmatter
from asdecided.core.fs import find_markdown_files
from asdecided.core.operations import CancelToken, OperationCancelled, Progress
from asdecided.core.schema import SchemaReference, available_schemas, schema_reference
from asdecided.core.templates import TemplateNotFound, TemplateResourceMissing
...
from asdecided.services.improve import improve_file
from asdecided.services.ingest import ConversionError, UnsupportedDocument, ingest
from asdecided.services.repository import Artifact, Repository, load_repository
...
class ExplorerAdapter:
    ...
    def open_ref(self, ref: str) -> LookupState:
        """Exact lookup for `/open`, with `decided resolve` semantics (ADR-026).
    ...
    def search_rows(self, args: str) -> LookupState:
        """Search for `/find` and bare input, with `decided find` semantics.
    ...
    def governing_decisions(self, path: str) -> LookupState:
        ...
        Consumes the same `decisions_for_path` core the `decided decisions-for` CLI
    ...
    def schema_overview(self) -> tuple[tuple[str, str], ...]:
        ...
        facts `decided schema` reports (ADR-015). Needs no loaded repository.
        """
    ...
    def improvement_rows(self, path: str) -> tuple[RecommendationRow, ...]:
        ...
        Core's improve analysis (`decided improve`) rendered as findings rows
    ...
    def write_new(self, artifact_type: str, target: str) -> str:
        ...
        try:
            ...
            return "No repository identity found — run `decided init` first, then retry"
    ...
    def stats_state(self) -> StatsState:
        ...
        Built from Core's stats service — the same facts `decided stats` reports
    ...
    def resolve_link(self, href: str, source_path: str) -> str | None:
        ...
        Tries reference resolution (IDs and aliases, `decided resolve` semantics),
