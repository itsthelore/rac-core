...
SDK consumer can catch the whole family with a single ``except asdecided.RACError``
...
class RACError(Exception):
    ...
    (for example :class:`asdecided.services.create.OutputPathExists`) to handle a
