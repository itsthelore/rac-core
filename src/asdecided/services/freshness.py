...
ADR-032 kept every ``decided mcp`` tool call re-reading the whole repository, and
...
from asdecided.core.corpus import CorpusEntry, content_hash
...
from asdecided.services.parallel_build import (
    BuildStats,
    emit_build_timing,
...
class FreshnessTracker:
    ...
    def read_model(self, *, verify: bool = False) -> CorpusReadModel:
        ...
        # phases (parallel parse, derive, store write) and emit the DECIDED_TIMING
    ...
    def _open_base(self, corpus_hash: str) -> CorpusReadModel | None:
        from asdecided.services.index_store import open_read_model
