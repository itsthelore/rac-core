...
and return them as advisory suggestions. ``decided doctor`` is the first surface;
``decided coverage`` is a candidate second one (left as a design open question).
...
from asdecided.services.index import index_from_corpus
from asdecided.services.relationships import RELATIONSHIP_SECTIONS, relationships_from_corpus
from asdecided.services.resolve import OUTCOME_RESOLVED, resolve_in_index
...
def detect_unlinked_references(
    ...
    ``entries`` lets a caller (such as ``decided doctor``) pass an already-walked
