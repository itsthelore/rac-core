...
from asdecided.core.models import SearchSection
from asdecided.services.agent_rules import artifact_status, is_live_decision
from asdecided.services.index import IndexEntry, index_from_corpus
from asdecided.services.portfolio import portfolio_from_corpus
...
from asdecided.services.resolve import field_tokens_for_entries, live_decision_paths
...
from asdecided.services.scope import (  # noqa: PLC2701 - deliberate reuse; see note above
    GoverningDecision,
    ScopeLookupResult,
    _entry_covers,
    _normalize_query,
)
from asdecided.services.scope_paths import repository_root
...
CACHE_DIR_ENV = "DECIDED_CACHE_DIR"
...
def governing_decisions(scope_rows: list[ScopeRow], directory: str, path: str) -> ScopeLookupResult:
    ...
    Byte-identical to :func:`asdecided.services.scope.decisions_for_path` for the same
...
def default_cache_dir() -> Path:
    """The derived-cache directory: ``DECIDED_CACHE_DIR`` > ``$XDG_CACHE_HOME/rac/derived``.
    ...
    if not base:
        try:
            ...
            base = str(Path(tempfile.gettempdir()) / "decided-cache")
    return Path(base) / "decided" / CACHE_DIRNAME
...
class DerivedIndexCache:
    ...
    def load_or_build(
        ...
        from asdecided.services.freshness import corpus_hash_from_manifest, stat_scan
        ...
        from asdecided.services.parallel_build import build_derived_index_parallel, emit_build_timing
    ...
    def _write_store(self, corpus_hash: str, derived: DerivedIndex) -> bool:
        from asdecided.services.index_store import write_store
