"""Policy-aware unified enforcement — `decided gate` (v0.21.14, ADR-049 / ADR-063).

``decided gate`` is the single enforcement entry point: it runs validation,
...
its committed ``.decided/config.yaml`` (loaded by :mod:`asdecided.services.init`, which
...
from asdecided.services.init import load_enforcement_policy, load_overrides
...
from asdecided.services.validate import DirectoryValidation, validate_corpus
...
def _relationship_finding(issue: RelationshipIssue) -> tuple[str, str, str, int | None, str]:
    ...
    """
    from asdecided.output.sarif import _relationship_result

    # Reuse the SARIF result builder so the message and URI never drift from the
    # standalone `decided relationships --sarif` output (one formatting source).
...
def build_gate(
    ...
    When ``policy`` is None it is loaded from the directory's ``.decided/config.yaml``
