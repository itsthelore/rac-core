...
*segment* files (:mod:`asdecided.services.index_format`), memory-mapped and read by
...
from asdecided.core.models import Issue, SearchSection
from asdecided.services.derived_cache import DerivedIndex, ScopeRow
from asdecided.services.index import IndexEntry
from asdecided.services.index_format import (
    IndexedSegment,
    IndexFormatError,
    Reader,
    Writer,
...
from asdecided.services.relationships import Relationship
...
def _iter_segment_files(
    ...
    """Encode a :class:`~asdecided.services.derived_cache.DerivedIndex` to segment files.
...
class Fold:
    ...
    def search(
        ...
        *,
        ...
        """`decided find` search served from the postings, byte-identical to a walk.
...
class ReadModelView:
    ...
    Exposes the same surface as :class:`~asdecided.services.derived_cache.DerivedIndex`
    ...
    def search(
        ...
        *,
        ...
        """Postings-served `decided find` search — byte-identical to ``search_index``.
...
# =============================================================================
#
# `decided validate DIR --cache` reuses per-file validation results across runs. A
...
def _decode_validation_store(
    ...
    if stored_config != config_hash:
        # An ancestor `.decided/config.yaml` edit (or a different governing config
...
# =============================================================================
#
# The one-shot `decided find` cache path verifies freshness with the shared
...
def manifest_root_key(directory: str, *, recursive: bool = True) -> str:
    ...
    The recursion mode folds into the key because `decided find` and
    `decided find --top-level` walk different file sets from the same root: a shared
