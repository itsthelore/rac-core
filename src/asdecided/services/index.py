"""Repository index — `decided index` (v0.7.5).
...
to ``decided inspect`` / ``decided relationships`` / ``decided portfolio``.
...
class IndexEntry:
    ...
    # (v0.10.3): the body/heading tiers of `decided find` and their snippets read
...
def index_from_corpus(
    ...
    *,
    ...
    """
    if inbound is None:
        from asdecided.services.relationships import inbound_counts_from_corpus
