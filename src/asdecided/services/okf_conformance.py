...
conformance was only a side-effect of ``decided export --okf``; this check makes it
a write-time gate, surfaced through ``decided validate`` over a corpus.
...
from asdecided.core.okf import OKF_TYPE, RESERVED_FILENAMES
from asdecided.core.overrides import EMPTY, SeverityOverrides, resolve_severity
    ...
    for entry in entries:
        ...
        if artifact_type not in OKF_TYPE:
            _add(
                ...
                f"it to asdecided.core.okf.OKF_TYPE so the artifact is carried in the "
