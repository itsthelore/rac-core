"""Directory validation — `decided validate <directory>` (v0.7.9).
...
``decided validate``. Unknown-type files are reported as skipped, not failed —
the same semantics as ``decided portfolio`` (unknown is a valid outcome, and the
...
from asdecided.core.models import Issue, Product
from asdecided.core.overrides import SeverityOverrides, apply_overrides
...
class StdinCorpusValidation:
    ...
    The result of ``decided validate - --corpus DIR`` (v0.21.17, ADR-067): the
    ...
    not block, mirroring single-file ``decided validate``.
    """
...
def validate_stdin_against_corpus(
    ...
    The engine seam behind ``decided validate - --corpus DIR`` and the generated
    Claude Code ``PreToolUse`` pre-edit hook (v0.21.17, ADR-067): plain
    ``decided validate -`` is single-document and cannot resolve cross-artifact
    ...
    *missing* decision would slip through. This composes the two existing
    ...
    1. structural validation with the corpus' severity overrides applied
       ...
       normal ``decided validate`` in that repository, ADR-053); and
...
def validate_product(product: Product, start: str = ".") -> list[Issue]:
    ...
    ``.decided/config.yaml`` governs policy). The CLI's single-file ``decided validate``
...
def validate_corpus(
    ...
    loaded from the directory's ``.decided/config.yaml``, so the repository model
    behind review / watchkeeper / portfolio honours the same policy as
    ``decided validate``. Pass :data:`~asdecided.core.overrides.EMPTY` to opt out. Overrides
...
# =============================================================================
# Incremental directory validation — `decided validate DIR --cache` (ADR-106).
# =============================================================================
#
...
# OKF conformance is per-file. There is no cross-file layer in ``decided validate DIR``
# — duplicate-identifier / relationship-resolution / cycle checks live in the
# relationships subsystem (``decided relationships --validate`` / ``decided gate``), not
...
def _config_fingerprint(directory: str) -> str:
    """A fingerprint of the ancestor-walked ``.decided/config.yaml`` governing ``directory``.
...
def validate_directory_incremental(
    ...
    *,
    ...
    ``DECIDED_TIMING`` is set, one ``decided-timing:`` line is written to stderr with the
    ...
    from asdecided.services.freshness import FileState, stat_scan
    ...
    if timing:
        sys.stderr.write(
            f"decided-timing: detect_ms={detect_ns / 1_000_000:.3f} "
