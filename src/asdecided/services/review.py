"""Repository review — `decided review` (v0.7.9).
...
remains with ``decided relationships --validate``, which performs the repo-level
...
# The stable drift code is owned by `asdecided.services.drift`; re-exported here under
...
class ReviewReport:
    ...
    mirror the ``decided portfolio`` contract so consumers can share parsing.
    """
...
def _drift_findings(directory: str, entries: list[CorpusEntry]) -> list[ReviewIssue]:
    ...
    Surfaces the same git-native signal ``decided doctor`` reports, through review's
