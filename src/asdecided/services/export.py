"""Corpus export — `decided export` (v0.11.0).
...
- ``status`` is the first non-empty line of the artifact's ``## Status``
  ...
  as ``decided inspect`` does (so a decision's ``accepted`` exports as
  ``Accepted``). ``decided inspect`` *omits* a missing status; the export
...
- ``title`` is the document title; an untitled artifact falls back to its
  canonical identifier (``decided index`` emits ``null`` there, but the viewer
...
from asdecided.core.relationship_types import edge_spec
...
class DocumentsExport:
    ...
    artifact from AsDecided, and ``status`` so a retired decision is filterable on read.
    """
...
def build_corpus_export(directory: str, recursive: bool = True) -> CorpusExport:
    ...
    return CorpusExport(
        ...
        rac_version=asdecided.__version__,
        artifacts=artifacts,
        relationships=edges,
    )
...
def build_documents_export(directory: str, recursive: bool = True) -> DocumentsExport:
    ...
    Markdown body and the verify-in-AsDecided metadata rather than the viewer's HTML.
