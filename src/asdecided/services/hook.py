"""Git hook installation — `decided hook install` (v0.13.4).
...
``decided skill install`` (ADR-021 resource loading; never-overwrite posture).

Failure contract:

- unknown style          → :class:`~asdecided.core.hooks.HookNotFound` (usage error)
- no .git directory      → :class:`NotAGitWorkTree` (usage error, exit 2)
- hook file exists       → :class:`HookFileExists` (refused; exit 1; untouched)
- missing packaged hook  → :class:`~asdecided.core.hooks.HookResourceMissing`
...
from asdecided.core.hooks import DEFAULT_STYLE, HookNotFound, available_hooks, load_hook
...
class NotAGitWorkTree(RACError):
    ...
    def __init__(self, directory: str) -> None:
        ...
        super().__init__(
            f"no .git directory in {directory}; run `decided hook install` from a git repository root"
        )
...
@dataclass
class InstalledHook:
    """Result of a `decided hook install` run (stable JSON contract, ADR-007)."""
...
def install_hook(target_dir: str, style: str = DEFAULT_STYLE) -> InstalledHook:
    ...
    Raises :class:`~asdecided.core.hooks.HookNotFound` for an unknown style,
    :class:`NotAGitWorkTree` when there is no ``.git`` directory,
    :class:`HookFileExists` when the target hook already exists (left
    untouched), and :class:`~asdecided.core.hooks.HookResourceMissing` when the
