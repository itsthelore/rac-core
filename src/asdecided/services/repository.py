...
through ``decided index`` / ``decided validate`` / ``decided relationships`` /
``decided portfolio``.
...
from asdecided.core.corpus import CorpusEntry, collect_corpus
from asdecided.core.models import SearchSection
from asdecided.core.operations import CancelToken, Progress, ProgressCallback, checkpoint
...
class Artifact:
    ...
    artifact does not fill — the same completeness rules ``decided inspect`` and
    ``decided portfolio`` report; always empty for unknown artifacts.
    """
    ...
    # lets the repository model serve body-tier `decided find` searches through the
...
class Repository:
    ...
    def artifact(self, ref: str) -> Artifact | None:
        ...
        ambiguity is relationship validation's job, ranking is ``decided find``'s.
        """
        key = ref.casefold()
