...
`decided stats <directory>` walks the tree for Markdown files, parses and classifies
each one, and aggregates the results. Like the rest of RAC, it works on the
Product AST: every `.md` is parsed into a :class:`~asdecided.core.models.Product`.
...
class RoadmapStat:
    ...
    or quality breakdowns are intentionally absent — those belong to `decided improve`,
...
class PortfolioStats:
    ...
    def has_meaningful_content(self) -> bool:
        ...
        empty nor meaningful. ``decided stats`` exits OK when this is true *or* the
