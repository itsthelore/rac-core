...
`decided improve <file>` is RAC's *advisory* capability: it reports which required and
...
(:func:`asdecided.core.classification.missing_sections`); it never reaches into classification
...
from asdecided.core.classification import classify, missing_sections
...
def supports_improve(spec: ArtifactSpec) -> bool:
    ...
    Gates ``decided improve`` so a future artifact type cannot become improvable until
