...
* **Resolution reuses the one identity model.** ``old_ref`` is resolved against
  ...
  (:func:`asdecided.services.relationships.build_resolution_index`), so "what does
...
from asdecided.core.identity import artifact_identifiers
...
def _identity_edit(
    ...
    :func:`asdecided.core.identity.artifact_identifier` for the *declared* (editable)
...
def _section_first_value_edit(
    ...
    (the same first-value rule :func:`asdecided.core.identity._first_value` uses). The
