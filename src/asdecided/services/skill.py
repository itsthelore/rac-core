"""Skill installation — `decided skill install` (v0.10.5).
...
- unknown skill name      → :class:`~asdecided.core.skills.SkillNotFound` (usage
...
- missing packaged skill  → :class:`~asdecided.core.skills.SkillResourceMissing`
...
from asdecided.core.skills import SkillNotFound, available_skills, load_skill
...
class SkillInstallation:
    """Result of a `decided skill install` run (stable JSON contract, ADR-007)."""
...
def install_skills(target_dir: str, skill_name: str | None = None) -> SkillInstallation:
    ...
    Raises :class:`~asdecided.core.skills.SkillNotFound` for an unregistered name,
    :class:`SkillFileExists` when any target file already exists, and
    :class:`~asdecided.core.skills.SkillResourceMissing` when a packaged resource
    is absent.
    """
