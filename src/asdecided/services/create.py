"""Artifact creation — `decided new` (v0.7.10, identity added in v0.7.11).
...
read from the nearest ``.decided/config.yaml`` (``decided init``), one ID is
...
- unsupported type          → :class:`~asdecided.core.templates.TemplateNotFound` (usage)
...
- missing packaged template → :class:`~asdecided.core.templates.TemplateResourceMissing`
  (operational)
"""
...
from asdecided.services.init import load_repository_config
...
class MissingRepositoryConfig(RACError):
    """No repository identity namespace is established (run `decided init`)."""
