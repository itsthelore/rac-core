...
``decided inspect`` (which exposes them as the additive ``relationships`` field) and
``decided stats`` (which counts their presence).
...
from asdecided.core.artifacts import ArtifactSpec, spec_for
from asdecided.core.classification import classify
from asdecided.core.corpus import CorpusCache, CorpusEntry, walk_corpus
from asdecided.core.identity import artifact_identifier, artifact_identifiers
from asdecided.core.limits import (
    MAX_RELATED_EDGES,
    MAX_TRAVERSAL_DEPTH,
...
# :mod:`asdecided.services.references`. They are re-exported here so the module remains
# the single import surface every relationship consumer has always used
# (``asdecided.services.relationships.RELATIONSHIP_SECTIONS`` and friends resolve
# unchanged, ADR-062).
from asdecided.services.references import EXTERNAL_SECTIONS as EXTERNAL_SECTIONS
from asdecided.services.references import RELATED_SECTIONS as RELATED_SECTIONS
from asdecided.services.references import (
    RELATIONSHIP_SECTIONS,
    _snake,
    extract_relationships_full,
    unsupported_relationship_sections,
)
from asdecided.services.references import SCOPE_SECTIONS as SCOPE_SECTIONS
from asdecided.services.references import extract_relationships as extract_relationships
from asdecided.services.references import parse_references as parse_references
from asdecided.services.references import (
...
# :mod:`asdecided.services.scope_paths`, a home shared with ``asdecided.services.scope`` so
...
#
# `decided relationships <path>` discovers the explicit relationships declared across
...
class ArtifactRelationships:
    ...
    ``relationships`` includes Supersedes (unlike ``decided inspect``) and is keyed by
...
#
# `decided relationships <path> --validate` resolves every explicit reference against
...
# and the `decided gate` enforcement layer both read it, so they can never disagree.
...
def _is_retired_artifact(product: Product, spec: ArtifactSpec | None) -> bool:
    ...
    ``decided inspect`` uses, inlined to avoid importing ``inspect`` (which imports
    this module).
    """
...
# live in :mod:`asdecided.services.scope_paths`, shared with ``asdecided.services.scope``.
...
def validate_document_against_corpus(
    ...
    ``decided validate - --corpus`` from the hook before the edit lands — has its
...
#
# Aggregate relationship health for ``decided portfolio``. Returns counts and an
...
class RelationshipSummary:
    ...
    findings (``broken == len(issues)``); consumers like ``decided portfolio`` turn
...
#
# The repository model (asdecided.services.repository) needs every declared reference
...
def inbound_counts_from_corpus(entries: list[CorpusEntry]) -> dict[str, int]:
    ...
    (the same definition ``decided doctor``'s orphan/hub pass and the search graph
