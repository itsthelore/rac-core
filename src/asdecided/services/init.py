"""Repository identity configuration — `decided init` (v0.7.11).

``decided init`` establishes the repository identity namespace (ADR-026): a
``.decided/config.yaml`` holding the ``repository_key`` that prefixes every
...
configuration found, so ``decided new`` works from any subdirectory of an
...
if TYPE_CHECKING:
    from asdecided.services.gate import EnforcementPolicy

from asdecided.core.overrides import EMPTY, RULE_VALUES, TYPE_VALUES, SeverityOverrides
from asdecided.core.validation import TICKETING_PROVIDER_NAMES
...
class RepositoryConfig:
    ...
    repository_key: str
    config_path: str  # the .decided/config.yaml this came from
...
def _parse_config_yaml(config_path: Path) -> object:
    ...
    The single point that reads ``.decided/config.yaml`` and turns a
...
def find_config_file(start_dir: str) -> Path | None:
    """The nearest ``.decided/config.yaml`` at or above ``start_dir``, or None.
...
def load_overrides(start_dir: str) -> SeverityOverrides:
    ...
    Returns :data:`~asdecided.core.overrides.EMPTY` when there is no config file or no
...
def load_enforcement_policy(start_dir: str) -> EnforcementPolicy:
    ...
    ``decided gate``: three optional list-of-strings keys ``blocking``, ``advisory``,
    ...
    Returns :data:`~asdecided.services.gate.EMPTY_POLICY` when there is no config file or
    ...
    from asdecided.services.gate import EMPTY_POLICY, EnforcementPolicy
...
def init_repository(
    ...
    initialized repository is left untouched (edit ``.decided/config.yaml`` to change
    a provider later).

    When ``profile`` is given (a built-in profile name, ADR-088), the profile's
    config-only bundle is layered on a fresh init: its ``.decided/config.yaml``
    ...
    *configuration only*, never authored prose (ADR-024, ADR-044, ADR-085).
    ...
    leave unchanged. ``.decided/config.yaml`` is untouched by it on either path.
    ...
    :class:`asdecided.services.profiles.MalformedClientConfig` when an existing
