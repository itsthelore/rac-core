...
Either answer to the ``decided init`` prompt is persisted — including a decline —
so the question is asked at most once per machine: :func:`consent_recorded`
is the gate, not the answer.

This module lives outside ``asdecided.mcp`` because importing that package pays the
MCP SDK import, and ``decided init`` / ``decided telemetry`` must stay SDK-free. The
PostHog constants live here for the same reason — they are inert strings; the
only network code in RAC is :mod:`asdecided.mcp.ping`.
"""
...
# `decided telemetry status` says so. The sink is swappable behind these two
# constants.
...
def consent_path() -> Path:
    ...
    return Path(base) / "decided" / CONSENT_FILENAME
