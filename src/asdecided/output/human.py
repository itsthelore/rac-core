...
Keeping rendering out of :mod:`asdecided.cli` lets the CLI stay thin and makes the
output formats easy to test directly. JSON lives in :mod:`asdecided.output.json` and
Markdown templates in :mod:`asdecided.output.templates`.
"""
...
from asdecided.core.classification import CONFIDENCE_THRESHOLD, TypeScore
...
from asdecided.services.compare import (
    CHANGE_ADDED,
    CHANGE_MODIFIED,
    CHANGE_REMOVED,
...
from asdecided.services.validate import STATUS_INVALID, DirectoryValidation, StdinCorpusValidation
...
def render_stdin_corpus_human(result: StdinCorpusValidation) -> str:
    """Human-readable `decided validate - --corpus` output (v0.21.17, ADR-067).

    Renders the proposed document's structural findings exactly as single-file
    ``decided validate`` does, then appends the corpus-resolved relationship findings
...
def render_validate_dir_human(result: DirectoryValidation) -> str:
    """Human-readable directory `decided validate` output (v0.7.9).
...
def render_portfolio_human(s: PortfolioSummary) -> str:
    """Human-readable `decided portfolio` output."""
    lines = [
...
def render_review_human(r: ReviewReport) -> str:
    """Human-readable `decided review` output (v0.7.9).
...
def render_gate_human(report: GateReport) -> str:
    """Human-readable `decided gate` output (v0.21.14).
    ...
    not. The style mirrors `decided review` — coloured icons, a green clear line.
    """
...
def render_index_human(index: RepositoryIndex) -> str:
    """Human-readable `decided index` output: a repository manifest."""
    lines = [
...
def render_templates_human(names: list[str]) -> str:
    """Human `decided templates` output: the canonical template set."""
...
def render_new_human(created: CreatedArtifact) -> str:
    """Human `decided new` output: what was created, its identity, and where."""
    return (
        f"Created {created.artifact_type} artifact: {created.path}\n"
        f"ID: {created.id}\n"
        f"\n"
        f"Edit the TODO placeholders, then check it with: decided validate {created.path}"
    )


def render_init_human(result: InitResult) -> str:
    """Human `decided init` output: the established identity namespace."""
...
def render_quickstart_human(result: QuickstartResult) -> str:
    """Human `decided quickstart` output: identity, first artifact, next step."""
    ...
    return (
        ...
        f"ID: {artifact.id}\n"
        f"\n"
        f"Next: edit the TODO placeholders, then run: decided validate {artifact.path}"
    )
...
def render_resolve_human(result: ResolutionResult) -> str:
    """Human `decided resolve` output for a resolved artifact."""
    artifact = result.artifact
...
def render_decisions_for_human(result: ScopeLookupResult) -> str:
    """Human `decided decisions-for` output: the decisions governing a path.
...
def render_find_human(result: SearchResult, *, explain: bool = False) -> str:
    """Human `decided find` output: aligned match rows, or a valid empty result.
...
def render_migrate_human(report: MigrationReport) -> str:
    """Human `decided migrate metadata` output: assigned IDs and what remains."""
    lines: list[str] = []
    if report.dry_run:
...
def render_skill_install_human(installation: SkillInstallation) -> str:
    """Human `decided skill install` output: what was installed and where."""
...
def render_skill_list_human(specs: list[SkillSpec]) -> str:
    """Human `decided skill list` output: the bundled skill set."""
...
def render_hook_install_human(installation: InstalledHook) -> str:
    """Human `decided hook install` output: what was installed and where."""
    h = installation
    return (
...
def render_hook_list_human(specs: list[HookSpec]) -> str:
    """Human `decided hook list` output: the bundled hook set."""
...
def render_mcp_stats_human(summary: MCPTelemetrySummary) -> str:
    """Human `decided mcp-stats` output — what the local telemetry log says.
...
# Per-file state icons for `decided export --agent-rules` (v0.21.15). A drift state
...
def render_agent_rules_human(result: AgentRulesResult) -> str:
    """Human-readable `decided export --agent-rules [--check]` output (v0.21.15).
...
def render_vault_ingest_human(
    ...
    """Human summary for `decided ingest <dir>`: what converted, what needs review.
