"""SARIF rendering for `decided validate` — CI code scanning (ADR-054).

SARIF 2.1.0 is the format GitHub Code Scanning ingests to annotate a pull
request inline. `decided validate <dir> --sarif` emits one SARIF run covering both
...
from asdecided.services.validate import DirectoryValidation
...
def render_review_sarif(report: ReviewReport) -> str:
    """Render a `decided review` report as a SARIF 2.1.0 document (ADR-054).
...
# (``RELATIONSHIP_SEVERITY``), so the SARIF annotation level and the `decided gate`
...
def render_relationships_sarif(validation: RelationshipValidation) -> str:
    """Render `decided relationships --validate` as a SARIF 2.1.0 document (ADR-054).
...
def render_gate_sarif(report: GateReport) -> str:
    """Render a `decided gate` report as a single SARIF 2.1.0 document (v0.21.14).
