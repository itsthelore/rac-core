...
:class:`~asdecided.services.freshness.FreshnessTracker` keeps the derived read-model
...
With ``--no-cache`` (or ``DECIDED_NO_CACHE``) every tool call re-reads the
...
:mod:`asdecided.mcp.budget`).

Failed lookups return structured error data, never protocol exceptions
(ADR-034, :mod:`asdecided.mcp.errors`): an agent recovers from a JSON body.

Opt-in telemetry (v0.10.4, ADR-040): when serving with a recorder, each tool
call routes through :func:`asdecided.mcp.telemetry.observe`, which times the call,
...
``decided telemetry on`` (or the ``decided init`` prompt), ``run_server`` starts the
daily-ping daemon thread (:mod:`asdecided.mcp.ping`) — at most one pinned,
...
from asdecided import consent as consent_record
from asdecided.core.corpus import walk_corpus
from asdecided.core.limits import MAX_TRAVERSAL_DEPTH
from asdecided.core.markdown import parse
from asdecided.mcp import audit, errors, ping, telemetry, transport
...
from asdecided.mcp.telemetry import TelemetryRecorder
...
from asdecided.services.index import build_repository_index, index_from_corpus
from asdecided.services.index_store import ReadModelView
from asdecided.services.recency import annotate_search_recency, artifact_provenance
...
def _request_principal(ctx: Context) -> str | None:
    """The caller's asserted principal from the ``X-AsDecided-Principal`` header, or None.
...
def build_server(
    ...
    :class:`~asdecided.services.freshness.FreshnessTracker` keeps the derived structures
    ...
    # DECIDED_NO_CACHE) every closure below builds fresh from disk on each call,
    # exactly ADR-032.
...
def run_server(
    ...
    (ADR-112) — the CLI's ``--no-cache`` / ``DECIDED_NO_CACHE`` pass ``False``,
    ...
    # by an ``audit:`` stanza in .decided/config.yaml. Default-absent — no stanza,
    ...
    if cache_enabled:
        ...
        print(
            ...
            "--no-cache or DECIDED_NO_CACHE=1 disables).",
            file=sys.stderr,
        )
    server = build_server(
