...
apart from the read-only tool logic in :mod:`asdecided.mcp.server`. The serving ADR
...
class AuditSinkUnavailable(RACError):
    ...
    ``audit:`` stanza in ``.decided/config.yaml``.
    """
...
def ensure_audit_sink(recorder: audit.AuditRecorder | None) -> None:
    ...
    if recorder is None:
        raise AuditSinkUnavailable(
            ...
            ".decided/config.yaml before serving over HTTP (ADR-084)."
        )
    try:
