...
- [REQ-001] RAC MUST provide a compound retrieval operation `retrieve_grounding(task, scope=None, top_k=5, budget=None, live_only=True)` as an MCP tool and `rac retrieve "<task>" [directory] [--scope <path>] [--top-k K] [--budget N] [--live | --all] [--json]` as a CLI subcommand, both thin faces over one shared core service function (ADR-031).
...
- [REQ-008] `search_artifacts` MUST gain an additive `live_only: bool = False` argument and `rac find` a `--live` flag, filtering pre-scoring by the spec-driven retired-status predicate for every artifact type; `get_artifact` MUST gain an additive per-call `budget: int | None = None` argument with REQ-007 semantics. Calls without the new arguments MUST return byte-identical responses to today (ADR-007), and existing `find_decisions` behaviour MUST be unchanged.
