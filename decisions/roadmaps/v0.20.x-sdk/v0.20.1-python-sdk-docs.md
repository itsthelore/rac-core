...
standalone example sub-project (mirroring the `itsthelore/decisiongrounding`
benchmark) that depends on `requirements-as-code` as a real package.
...
mirroring the `itsthelore/decisiongrounding` benchmark: its own `pyproject.toml`,
README, LICENSE, Makefile, and `tests/`, structured so it can graduate into its
own repository later without rework. Unlike `itsthelore/decisiongrounding` (which
treats `rac`
...
- A standalone example sub-project (`itsthelore/decisiongrounding`-style: own `pyproject.toml`
...
- A self-contained sub-project duplicates packaging scaffolding inside the
  monorepo. Accepted: this is the established `itsthelore/decisiongrounding` pattern and is
