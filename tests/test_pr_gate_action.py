"""Structural tests for the AsDecided PR-gate composite action.
...
def test_action_is_composite():
    ...
    assert a["name"] == "AsDecided PR gate"


def test_action_declares_expected_inputs():
    inputs = _action()["inputs"]
    for name in ("path", "upload-sarif", "sarif-dir", "decided-version", "install-from"):
...
def test_action_runs_single_gate_command():
    ...
    assert "decided relationships" not in run_steps
    assert "decided review" not in run_steps
...
def test_action_uploads_single_sarif_once():
    ...
    assert uploads[0]["with"]["category"] == "decided-gate"
