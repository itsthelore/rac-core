"""Tests for asdecided.core.skills, asdecided.services.skill, and the `decided skill` CLI.

Pins the v0.10.5 bundled-skill contract (REQ-005..008 of
rac/requirements/decided-growth-agent-skill.md): every bundled skill ships as a
package resource byte-identical to the repository's dogfood copy under
`.claude/skills/`, `decided skill install` writes all skills (all-or-nothing) or
one by name to the documented Claude Code discovery path without ever
overwriting, `decided skill list` enumerates the bundle, and exit codes follow
...
from asdecided.core.skills import (
    BUNDLED_SKILLS,
    SkillNotFound,
...
SKILL_NAMES = ["decided-artifacts", "decided-review", "decided-import", "decided-capture"]
...
def test_unknown_skill_raises_skill_not_found():
    ...
    with pytest.raises(SkillNotFound, match="decided-artifacts, decided-review, decided-import"):
        load_skill("nonexistent")
...
def test_missing_resource_raises_skill_resource_missing(monkeypatch):
    ...
    from asdecided.core import skills as core_skills

    ghost = core_skills.SkillSpec(name="decided-ghost", description="No resource ships for this.")
    monkeypatch.setattr(core_skills, "BUNDLED_SKILLS", (*BUNDLED_SKILLS, ghost))
    with pytest.raises(SkillResourceMissing, match="packaged skill missing: decided-ghost"):
        core_skills.load_skill("decided-ghost")
...
def test_install_all_refuses_all_or_nothing_when_one_exists(tmp_path):
    ...
    dest = tmp_path / install_rel_path("decided-review")
    ...
    for name in ("decided-artifacts", "decided-import"):
...
def test_named_install_writes_exactly_one_skill(tmp_path):
    installation = install_skills(str(tmp_path), "decided-review")
    assert [s.skill for s in installation.skills] == ["decided-review"]
    assert (tmp_path / install_rel_path("decided-review")).read_bytes() == load_skill("decided-review")
    for name in ("decided-artifacts", "decided-import"):
        assert not (tmp_path / install_rel_path(name)).exists()


def test_named_install_never_overwrites(tmp_path):
    dest = tmp_path / install_rel_path("decided-import")
    dest.parent.mkdir(parents=True)
    dest.write_text("precious user content", encoding="utf-8")
    with pytest.raises(SkillFileExists, match="never overwrites"):
        install_skills(str(tmp_path), "decided-import")
    assert dest.read_text(encoding="utf-8") == "precious user content"


def test_named_install_unknown_skill_raises(tmp_path):
    with pytest.raises(SkillNotFound, match="unknown skill: decided-nope"):
        install_skills(str(tmp_path), "decided-nope")
    assert not (tmp_path / ".claude").exists()
...
def test_cli_named_install_creates_exactly_one_file(tmp_path, capsys):
    rc = main(["skill", "install", "decided-review", "--dir", str(tmp_path)])
    assert rc == 0
    assert (tmp_path / install_rel_path("decided-review")).read_bytes() == load_skill("decided-review")
    for name in ("decided-artifacts", "decided-import"):
        assert not (tmp_path / install_rel_path(name)).exists()
    assert "Installed decided-review skill" in capsys.readouterr().out


def test_cli_unknown_skill_name_exits_2_and_lists_available(tmp_path, capsys):
    with pytest.raises(SystemExit) as exc:
        main(["skill", "install", "decided-nope", "--dir", str(tmp_path)])
    assert exc.value.code == 2
    err = capsys.readouterr().err
    assert "unknown skill: decided-nope" in err
    assert "decided-artifacts, decided-review, decided-import" in err
    assert not (tmp_path / ".claude").exists()
...
def test_cli_skill_install_missing_resource_is_operational_error(tmp_path, capsys, monkeypatch):
    def boom(target_dir, skill_name=None):
        raise SkillResourceMissing("decided-artifacts")

    monkeypatch.setattr("asdecided.cli.install_skills", boom)
    rc = main(["skill", "install", "--dir", str(tmp_path)])
    assert rc == 1
...
def test_cli_skill_list_rejects_a_skill_name(capsys):
    with pytest.raises(SystemExit) as exc:
        main(["skill", "list", "decided-review"])
    assert exc.value.code == 2
...
# refreshed with DECIDED_UPDATE_GOLDEN=1), but run from a tmp directory: `skill
...
GOLDEN_CASES = [
    ...
    ("skill_install_named_human", ["skill", "install", "decided-review"], 0),
    ("skill_install_named_json", ["skill", "install", "decided-review", "--json"], 0),
...
def test_skill_golden(name, argv, expected_rc, capsys, monkeypatch, tmp_path):
    ...
    assert out == golden.read_text(encoding="utf-8"), (
        ...
        "DECIDED_UPDATE_GOLDEN=1 python -m pytest tests/test_skill.py"
    )
