...
| `class-b-read-cap.md` | B — `RAC_MAX_FILE_BYTES` read-crash zones (any readable file) | `OverflowError: cannot fit 'int' into an index-sized integer` (cap >= 2^63-1) / `OverflowError: byte string is too large` (2^63-34 .. 2^63-2) | `FileCap::OracleCrash` marker at the read stage; incremental reads never preallocate the cap |
