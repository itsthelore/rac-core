...
//! (ADR-098's mandatory-audit-on). Mirrors `src/rac/mcp/audit.py`: one JSON
...
//! configured `.rac/config.yaml` audit sink.
//!
...
const PRINCIPAL_ENV: &str = "RAC_AUDIT_PRINCIPAL";
...
/// The audit principal: `RAC_AUDIT_PRINCIPAL` > (git identity, if allowed) >
        ...
        match std::fs::OpenOptions::new().create(true).append(true).open(&self.path) {
            ...
            Err(e) => {
                if !self.warned {
                    ...
                    eprintln!(
                        "rac mcp: audit write failed ({e}); {action} \
